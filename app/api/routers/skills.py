...
@router.get("/{skill_id}")
def get_skill(skill_id: UUID7, crud: SkillCRUD = Depends()):
    selected = crud.select(Skill.id == skill_id)
    return model_to_dto(selected, SkillSchemaRead)

@router.patch("/{skill_id}")
def update_skill(data: SkillSchemaPatch, skill_id: UUID7, crud: SkillCRUD = Depends()):
    info = data.model_dump(exclude_unset=True)
    updated = crud.update(Skill.id == skill_id, **info)
    return model_to_dto(updated, SkillSchemaRead)

@router.delete("/{skill_id}")
def delete_skill(skill_id: UUID7, crud: SkillCRUD = Depends()):
    deleted = crud.delete(Skill.id == skill_id)
