...
async def incorrect_password(request: Request, exc: IncorrectPasswordError):
    return JSONResponse(status_code=401, content={"detail": "Incorrect old password"})
...
async def task_already_done(request: Request, exc: TaskAlreadyDoneError):
    return JSONResponse(status_code=404, content={"detail": "Task repeat limit is 0"})
...
async def username_already_exists(request: Request, exc: UsernameAlreadyTakenError):
    return JSONResponse(status_code=404, content={"detail": "Username already taken"})
