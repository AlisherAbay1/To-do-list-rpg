...
class GetStatsOverviewInteractor:
    ...
    async def __call__(self, session_token: str) -> StatsOverviewDTO:
        ...
        total_completed = (
            await self.task_history_repo.get_amount_of_total_completed_tasks(user_id)
        )
        completed_today = (
            await self.task_history_repo.get_amount_of_today_completed_tasks(user_id)
        )
        completed_this_week = (
            await self.task_history_repo.get_amount_of_this_week_completed_tasks(
                user_id
            )
        )
