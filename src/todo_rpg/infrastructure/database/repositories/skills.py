...
from todo_rpg.domain import Skill, Task
...
class SkillRepository:
    ...
    async def get_skills_by_user_id(
        ...
        skills = (
            select(Skill).where(Skill.user_id == user_id).limit(limit).offset(offset)
        )
        result = await self._session.scalars(skills)
        return result.all()
    ...
    async def get_skills_by_lvl_desc_order(self, user_id: UUID) -> Sequence[Skill]:
        skills = (
            select(Skill).where(Skill.user_id == user_id).order_by(Skill.lvl.desc())
        )
        result = await self._session.scalars(skills)
        return result.all()
