...
from todo_rpg.presentation.mappers import TaskSchemaMapper
...
async def get_current_user_task(
    ...
    return await interactor(
        session_token, task_id, get_related_skills, get_related_items
    )


@router.post("/me", response_model=TaskSchemaReadable)
async def create_current_user_task(
    interactor: FromDishka[CreateCurrentUserTaskInteractor],
...
async def complete_task(
    interactor: FromDishka[CompleteTaskInteractor],
    task_id: UUID7,
    session_token=Cookie(None),
):
    if session_token is None:
...
async def uncomplete_task(
    interactor: FromDishka[UncompleteTaskInteractor],
    task_id: UUID7,
    session_token=Cookie(None),
):
    if session_token is None:
...
async def patch_task(
    interactor: FromDishka[UpdateCurrentUserTaskInteractor],
    task_id: UUID7,
