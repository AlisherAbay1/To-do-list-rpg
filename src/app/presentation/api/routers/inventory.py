...
async def get_current_user_inventory_items(
    interactor: FromDishka[GetCurrentUserInventoryItemsInteractor],
...
async def get_current_user_inventory_item_by_id(
    interactor: FromDishka[GetCurrentUserInventoryItemByIdInteractor],
...
@router.post(
    "/me/{inventory_item_id}/use", response_model=InventoryShortWithItemSchemaRead
)
async def use_current_user_inventory_item_by_id(
    interactor: FromDishka[UseCurrentUserInventoryItemInteractor],
    inventory_item_id: UUID,
    session_token=Cookie(None),
):
    if session_token is None:
...
async def delete_current_user_inventory_item(
    inventory_item_id: UUID,
