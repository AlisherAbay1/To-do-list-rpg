...
class GetCurrentUserTaskCategoryById:
    ...
    async def __call__(self, session_token: str, task_category_id: UUID, get_tasks: bool) -> TaskCategoryWithTasksDTO:
        user_id = await self.cash_repo.get_user_id_by_session_token(session_token)
        if user_id is None: 
            raise SessionNotFoundError()
        task_category = await self.task_category_repo.get_task_category_by_id(task_category_id)
