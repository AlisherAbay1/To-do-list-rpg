...
class GetCurrentUserShopListingsInteractor:
    ...
    async def __call__(self, session_token: str, limit: int, offset: int) -> list[ShopListingShortDTO]:
        user_id = await self.cash_repo.get_user_id_by_session_token(session_token)
        if user_id is None:
            raise SessionNotFoundError()
        shop_listings = await self.repo.get_shop_listings_by_user_id(user_id, limit, offset)
