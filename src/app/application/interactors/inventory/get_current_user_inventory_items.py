...
class GetCurrentUserInventoryItemsInteractor:
    ...
    async def __call__(self, session_token: str, limit: int, offset: int) -> list[InventoryShortDTO]:
        user_id = await self.cash_repo.get_user_id_by_session_token(session_token)
        if user_id is None:
            raise SessionNotFoundError()
        inventory_items = await self.repo.get_inventory_items_by_user_id(user_id, limit, offset)
